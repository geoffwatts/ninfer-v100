...
void HttpServer::handle_completions(const httplib::Request& req, httplib::Response& res) {
    ...
    PreparedRequest prepared;
    try {
        prepared = service_->prepare_completion(
            request, [&req] { return req.is_connection_alive && !req.is_connection_alive(); });
    } catch (const ApiException& e) {
        log_request_rejected(make_request_rejection_log_context(req_id, "openai_completions",
                                                                request, e.error()));
        write_error(res, e.error());
        return;
    } catch (const std::exception& e) {
    ...
    res.set_chunked_content_provider(
        "text/event-stream",
        [this, stream, id, created, model, include_usage, log_context](std::size_t,
            ...
            try {
                ...
                output.on_content = [&](const std::string& text) {
                    write_stream_item(
                        sink, *stream,
                        make_completion_chunk(id, model, created, text, include_usage));
                };
                output.on_reasoning = [&](const std::string& text) {
                    // Raw completions have no reasoning channel: any reasoning bytes join the
                    // visible text rather than a separate field.
                    write_stream_item(
                        sink, *stream,
                        make_completion_chunk(id, model, created, text, include_usage));
                };
                ...
                if (!remaining.empty()) {
                    write_stream_item(
                        sink, *stream,
                        make_completion_chunk(id, model, created, std::string(remaining),
                                              include_usage));
                }
                write_stream_item(
                    sink, *stream,
                    make_completion_chunk_final(id, model, created,
                                                finish_reason_wire(outcome.finish_reason),
                                                include_usage));
                if (include_usage) {
                    const CompletionUsage usage{outcome.prompt_tokens, outcome.completion_tokens};
