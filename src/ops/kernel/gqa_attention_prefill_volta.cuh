...
__launch_bounds__(128, 2) __global__ void gqa_attention_small_t_tc_volta_partial_kernel(
    ...
    __shared__ __align__(16) half q_s[Br * D];
    __shared__ __align__(16) half k_s[Bc * D];
    __shared__ __align__(16) half v_s[Bc * D];
    ...
    for (int row_base = 0; row_base < row_count; row_base += Br) {
        ...
        for (int idx = tid; idx < Br * D; idx += Threads) {
            ...
            q_s[row * D + d] = __float2half(__bfloat162float(value));
        }
        __syncthreads();
...
#pragma unroll
        ...
        for (int kb = 0; kb < key_blocks; ++kb) {
            ...
            for (int chunk = tid; chunk < Bc * (D / 8); chunk += Threads) {
                ...
                half* k_dst   = &k_s[key_l * D + d]; // raw bf16 bytes land here; converted below
                half* v_dst   = &v_s[key_l * D + d];
            ...
            for (int idx = tid; idx < Bc * D; idx += Threads) {
                const float kf = __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(k_s)[idx]);
                const float vf = __bfloat162float(reinterpret_cast<const __nv_bfloat16*>(v_s)[idx]);
                k_s[idx]       = __float2half(kf);
                v_s[idx]       = __float2half(vf);
            }
            __syncthreads();
...
#pragma unroll
                for (int c = 0; c < DChunks; ++c) {
                    half2 qf[4];
                    volta_load_qp(qf, reinterpret_cast<const half2*>(&q_s[c * 8]), D / 2);
                    half2 kf[4];
                    volta_load_k(
                        kf, reinterpret_cast<const half2*>(&k_s[sub * 8 * D + c * 8]), D / 2);
                ...
                for (int c = 0; c < DChunksLocal; ++c) {
                    ...
                    volta_load_v(vf,
                                 reinterpret_cast<const half2*>(
                                     &v_s[sub * 8 * D + dim_warp * DSlice + c * 8]),
                                 D / 2);
